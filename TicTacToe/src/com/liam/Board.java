package com.liam;

import java.util.Random;
import java.util.Date;

public class Board {
    private static CELL_STATE[][] Cells = new CELL_STATE[3][3];

    enum CELL_STATE { O, X, NONE }

    Board() {
        ClearBoard();
    }

    /**
     * Clear the game board
     */
    public void ClearBoard() {
        for (int x = 1; x <= 3; x++) {
            for (int y = 1; y <= 3; y++) {
                Cells[x - 1][y - 1] = CELL_STATE.NONE;
            }
        }
    }

    /**
     * Sets a cell specified by row and col to the given state.
     * Expects that row and col have already been checked for validity.
     * @param row    the row of the cell to be set
     * @param col    the column of the cell to be set
     * @param state  the state that the cell will be set to
     */
    public void SetCell(int row, int col, CELL_STATE state) {
        assert 1 <= row && row <= 3;
        assert 1 <= col && col <= 3;
        Cells[row - 1][col - 1] = state;
    }

    /**
     * Retrieves the string representation of the cell at the given position.
     * @param row    the row of the cell to be retrieved
     * @param col    the column of the cell to be retrived
     * @return string representing the state of the requested cell.
     */
    public String GetCell(int row, int col) {
        assert 1 <= row && row <= 3;
        assert 1 <= col && col <= 3;
        switch (Cells[row - 1][col - 1]) {
            case X: return "X";
            case O: return "O";
            case NONE: return " ";
        }
        return " ";
    }

    /**
     * Checks whether a cell is taken or not.
     * @param row    the row of the cell to be checked
     * @param col    the column of the cell to be checked
     * @return whether the cell is taken or not.
     */
    public boolean IsCellTaken(int row, int col) {
        assert 1 <= row && row <= 3;
        assert 1 <= col && col <= 3;
        return (Cells[row - 1][col - 1] != CELL_STATE.NONE);
    }

    /**
     * Checks whether a row is entirely filled or not.
     * @param row    the row to be checked
     * @return whether the row is filled or not.
     */
    public boolean IsRowFilled(int row) {
        assert 1 <= row && row <= 3;
        return Cells[row - 1][0] != CELL_STATE.NONE && Cells[row - 1][1] != CELL_STATE.NONE && Cells[row - 1][2] != CELL_STATE.NONE;
    }

    /**
     * Checks if the given player type has won.
     * @param row    the row of the cell to be checked
     * @param col    the column of the cell to be checked
     * @param player the type of player to check a win for
     * @return whether the player has won or not
     */
    public boolean CheckWin(int row, int col, CELL_STATE player) {
        return ((Cells[row][0] == player && Cells[row][1] == player && Cells[row][2] == player)
                || (Cells[0][col] == player && Cells[1][col] == player && Cells[2][col] == player)
                || (row == col && Cells[0][0] == player && Cells[1][1] == player && Cells[2][2] == player)
                || (row + col == 2 && Cells[0][2] == player && Cells[1][1] == player && Cells[2][0] == player));
    }

    /**
     * Makes a choice as an AI. Assumes the AI player is X.
     * @return: whether the AI won or not
     */
    public boolean ChooseAI() {
        Random Rand = new Random(new Date().getTime());
        while (true) {
            int x = Rand.nextInt(3);
            int y = Rand.nextInt(3);
            if (Cells[x][y] == CELL_STATE.NONE) {
                Cells[x][y] = CELL_STATE.X;
                return CheckWin(x, y, CELL_STATE.X);
            }
        }
    }
}